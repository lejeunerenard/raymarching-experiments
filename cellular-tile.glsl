...
float drawSphere(in vec4 p){
    ...
    p = cos(p*3.14159)*0.5; 
    p = abs(cos(p*3.14159)*0.5);
    return max(max(p.x - p.y, p.y - p.z), max(p.z - p.w, p.w - p.x));
    // return min(min(p.x - p.y, p.y - p.z), p.z - p.x);
}
...
float cellTile(in vec4 p) {
    // Draw four overlapping objects (spheres, in this case) at various positions throughout the tile.
    vec4 v, d; 
    d.x = drawSphere(p - vec4(.81, .62, .53, 0));
    p.xy = vec2(p.y-p.x, p.y + p.x)*.7071;
    d.y = drawSphere(p - vec4(.39, .2, .11, 0));
    p.yz = vec2(p.z-p.y, p.z + p.y)*.7071;
    d.z = drawSphere(p - vec4(.62, .24, .06, 0));
    p.xz = vec2(p.z-p.x, p.z + p.x)*.7071;
    d.w = drawSphere(p - vec4(.2, .82, .64, 0));
