...
vec3 getBackground (in vec2 uv, in float universe) {
  ...
  // vec3 color = mix(0.7 * vec3(0.3, 0.175, 0.175), 0.35 * vec3(0.175, 0.3, 0.3), bgIndex);
  vec3 color = mix(0.8 * vec3(0.85, 0.90, 0.90), vec3(1), bgIndex);
  ...
  color *= mix(vec3(1, 0.9, 1), vec3(0.8, 0.8, 1), 1. - 0.95 * (uv.y + 1.0));
  // color *= 1.2;
