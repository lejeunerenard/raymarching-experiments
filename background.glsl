...
vec3 getBackground (in vec2 uv, in float universe) {
  ...
  // --- Set colors / gradient ---
  // Gradients
  // vec3 color = mix(vec3(0.3, 0.25, 0.1), vec3(0.0), bgIndex);
  // vec3 color = mix(vec3(0.25, 0.2, 0.4), vec3(0.1), bgIndex);
  vec3 color = mix(vec3(1, 0.9, 0.95), vec3(0.6, 0.65, 0.7), bgIndex);
