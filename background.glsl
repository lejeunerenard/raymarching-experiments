...
vec3 getBackground (in vec2 uv, in float universe) {
  ...
  // vec3 color = mix(vec3(0.1), vec3(0.05), bgIndex);
  // vec3 color = mix(vec3(0.75), vec3(0.95, 0.95, 1), bgIndex);
  // vec3 color = mix(#14c2bf, #5070bF, bgIndex);
  ...
  // color *= 1.5;
  vec3 color = mix(vec3(0.4, 0.7125, 1), vec3(0.5), 0.30);
  ...
  // color *= vec3(0.75, 0.85, 1.00);
  // color = mix(color, vec3(0.5), 0.10); // desaturate
  color = mix(color, vec3(1), 0.4); // lighten
  // color *= 0.91;
