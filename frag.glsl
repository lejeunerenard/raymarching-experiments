...
vec3 map (in vec3 p, in float dT, in float universe) {
  ...
  float firstWave = dot(q, vec3(1, 0.33, -1) * q) - t + sin(q.z - q.y);
  firstWave = sin(TWO_PI * 2. * firstWave);
...
vec4 shade ( in vec3 rayOrigin, in vec3 rayDirection, in vec4 t, in vec2 uv, in float generalT ) {
    ...
    lights[1] = light(vec3( 1.5, 1.2,  1.0), #FFBBBB, 2.00);
    ...
    if (t.x>0. && backgroundMask > 0.) {
      ...
      // Material Types
      float isMountain = isMaterialSmooth(t.y, 0.);
...
vec3 two_dimensional (in vec2 uv, in float generalT) {
  ...
  // wQ += warpScale * 0.0125 * cos(19. * wQ.yx + 0. * localCosT );
...
vec4 sample (in vec3 ro, in vec3 rd, in vec2 uv) {
  ...
  //   const float maxDelayLength = 1.0;
  //   float layerT = norT
