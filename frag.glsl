...
#define distortT localCosT
  ...
  for (float i = 0.; i < num; i++) {
    float localR = r * ( 1. - 0.20 * i );
    vec3 localQ = q;
    localQ *= rotationMatrix(vec3(1), 0.3 * PI * cos(localCosT + 0.5 * PI * i / num));
    vec3 b = vec3(abs(length(localQ) - localR) - 0.105 * localR, 0, 0);
    // crop front plane
    float crop = localQ.z;
    b.x = max(b.x, crop);
    if (b.x < d.x) {
