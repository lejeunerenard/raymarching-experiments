...
vec3 map (in vec3 p, in float dT, in float universe) {
  ...
  wQ.xy *= rotMat2(0.30 * PI * cos(cosT + 2. * wQ.z));

  // To polar coordinates
  wQ = vec3(
      atan(wQ.y, wQ.x),
      scale * vmax(abs(wQ.xy)),
      wQ.z);
...
vec4 shade ( in vec3 rayOrigin, in vec3 rayDirection, in vec4 t, in vec2 uv, in float generalT ) {
    ...
    if (t.x>0. && backgroundMask > 0.) {
      ...
      float dispersionI = 2. * pow(1. - dot(nor, -rayDirection), 1.00);
...
vec3 two_dimensional (in vec2 uv, in float generalT) {
  ...
  vec2 axis = vec2(1);
  // float lineT = 10. * dot(q, vec2(1, 0));
  // float triangleAmount = 1.; // 0.5 + 0.5 * cos(2. * localCosT + lineT);
  // q += triangleAmount * 0.15 * size * triangleWave(1. * lineT);

  float n = dot(q, axis);
  n = sin(15. * TWO_PI * n);

  // float maskD = vmax(abs(q));
  float maskD = 0.75 * length(q);

  float mask = (0.5 + 0.5 * cos(TWO_PI * (t - maskD)));
  mask = pow(mask, 4.);
  n *= mask;
