...
vec3 baseColor (in vec3 pos, in vec3 nor, in vec3 rd, in float m, in float trap, in float t) {
  ...
  color = mix(vec3(0), vec3(0.05), isMaterialSmooth(m, 1.));
...
vec4 shade ( in vec3 rayOrigin, in vec3 rayDirection, in vec4 t, in vec2 uv, in float generalT ) {
    ...
    if (t.x>0. && backgroundMask > 0.) {
      ...
      reflectColor += 0.90 * mix(diffuseColor, vec3(1), 1.0) * reflection(pos, reflectionRd, generalT);
      color += reflectColor;
...
vec4 sample (in vec3 ro, in vec3 rd, in vec2 uv) {
  ...
  //   const float maxDelayLength = 0.07;
  //   float layerT = norT
