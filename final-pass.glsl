...
void main() {
  ...
  gl_FragColor.rgb = mix(gl_FragColor.rgb, colorBefore, 0.35);

  // Gamma encode
  gl_FragColor.rgb = pow(gl_FragColor.rgb, gammaEnc);

  // 'Film' Noise
  gl_FragColor.rgb += 0.025 * (cnoise2(560. * uv + sin(uv + time) + 200.0 * vec2(time, 0.0)) + cnoise2(800. * uv + 253.5 * vec2(0., time)));
}
